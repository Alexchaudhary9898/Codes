...
def apply(img, t):
    ...
    if t == 'NEG': return cv2.bitwise_not(img)
    if t == 'BLUR': return cv2.GaussianBlur(img,(15,15),0)
    return img

cap = cv2.VideoCapture(0)
if not cap.isOpened(): exit("❌ Webcam not found")
...
while True:
    ...
    if res.multi_hand_landmarks:
        for h in res.multi_hand_landmarks:
            ...
            for p in pts: cv2.circle(img,p,8,(255,0,255),cv2.FILLED)
            ...
            if abs(t[0]-i[0])<30 and abs(t[1]-i[1])<30 and now-last>delay:
                ...
                f=(f+1)%len(filters); last=now; print(f"🎨 Filter: {filters[f]}")

    img2 = apply(img, filters[f])
    cv2.imshow("🖐 Gesture Photo App", cv2.cvtColor(img2,cv2.COLOR_GRAY2BGR) if filters[f]=='GRAY' else img2)
